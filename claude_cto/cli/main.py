...
def start_server_in_background() -> bool:
    ...
    # Port discovery logic: finds next available port starting from 8888
...
def run(
    ...
    if not is_server_running(server_url):
        if not start_server_in_background():
            ...
            console.print("  2. Check if port 8888-8010 are available:")
            console.print("     [bright_white]$ lsof -i :8888[/bright_white]\n")
...
def server_start(
    ...
    port: Annotated[int, typer.Option("--port", "-p", help="Server port")] = 8888,
    ...
    if auto_port:
        ...
        for attempt in range(max_attempts):
            ...
            console.print(
                "[dim]Tip: Try specifying a different port with --port or stop the process using port 8888[/dim]"
            )
            raise typer.Exit(1)
