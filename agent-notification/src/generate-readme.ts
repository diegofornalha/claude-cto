...
async function generateText(prompt: string): Promise<string> {
  console.log("Gerando texto com a API do Gemini...");
  
  // Tenta cada modelo na lista até encontrar um que funcione
  for (const modelName of MODEL_NAMES) {
    try {
      ...
      console.log(`Modelo ${modelName} usado com sucesso!`);
      return response.text();
    } catch (error) {
      console.error(`Erro com o modelo ${modelName}:`, error);
      // Continua para o próximo modelo
    }
  }
  
  // Se nenhum modelo funcionou, lança um erro
  throw new Error("Não foi possível gerar texto com nenhum dos modelos disponíveis");
}
